...
void	render_map(void *mlx, void *win, char **map, Info *info)
{
	...
	while (map[y] != NULL)
	{
		...
		{
			if (map[y][x] == '1')
				mlx_put_image_to_window(mlx, win, info->wall_img, x * 50, y * 50);
			else if (map[y][x] == '0')
				mlx_put_image_to_window(mlx, win, info->floor_img, x * 50, y * 50);
			else if (map[y][x] == 'C')
				mlx_put_image_to_window(mlx, win, info->collectible_img, x * 50, y * 50);
			else if (map[y][x] == 'E')
				mlx_put_image_to_window(mlx, win, info->exit_img, x * 50, y * 50);
			else if (map[y][x] == 'P')
				mlx_put_image_to_window(mlx, win, info->player_img, x * 50, y * 50);
			x++;
		}
		y++;
	}
